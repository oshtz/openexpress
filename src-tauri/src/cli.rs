...
//! tool, pre-fill the selected files).
...
pub struct LaunchAction {
    ...
    /// file paths (e.g. Finder "Open With"), in which case the frontend shows
    /// a picker filtered by the selected files and batch capability.
    ...
    T: Into<OsString>,
{
    ...
    while let Some(arg) = args.next() {
        ...
        files.push(arg_text.into_owned());
    }

    match tool {
        Some(tool_id) => match tools::find(&tool_id) {
            Some(spec) => {
                if files.len() > 1 && !spec.accepts_multiple {
                    log::warn!("tool does not accept multiple files: {}", spec.id);
                    return Some(LaunchAction {
                        tool: None,
            ...
            None => {
                ...
                (!files.is_empty()).then_some(LaunchAction {
                    tool: None,
                    route: "/pick".to_string(),
                    files,
                })
        ...
        None => (!files.is_empty()).then_some(LaunchAction {
            tool: None,
            route: "/pick".to_string(),
            files,
        }),
...
mod tests {
    ...
    fn parses_tool_and_file() {
        ...
        assert_eq!(action.files, ["C:\\photo.jpg"]);
    }
    ...
    fn bare_file_routes_to_picker() {
        ...
        assert_eq!(action.files, ["/some/file.jpg"]);
    }
    ...
    fn parses_multiple_files() {
        ...
        .unwrap();
        assert_eq!(action.route, "/image/resize");
        assert_eq!(action.files, ["C:\\first.jpg", "C:\\second.png"]);
    }

    #[test]
    fn multiple_files_fall_back_to_picker_for_single_file_tool() {
        let action = parse([
            "openexpress",
            "--tool",
            "image-crop",
